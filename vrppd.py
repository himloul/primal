...
def create_data_model():
    ...
    data['vehicle_capacities'] = [4]*nv
    data['demands'] = demands
    data['pickups_deliveries'] = list_pd
    data['depot'] = 0
    return data
...
for j in range(0,len(color_index)-1):
  ...
  gmap.text(color_index[j][0], color_index[j][1]+0.02, 'Route ' + str(j) + ': '+str(routes_distance[j])+" m")
...
# Directions
for k in range(len(solution)):
    gmap.directions(solution[k][0],solution[k][len(solution[k])-2], waypoints = solution[k][1:(len(solution[k])-2)])
